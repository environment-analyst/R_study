...
```
> 여기서 컬럼내에서 data, information으로 저장되어 자료 가져오기(해당 행을 다 가져온다)</br>
...
```
> 이렇게 하면 컬럼명1의 내용에서 글자수가 1보다 작으면 결측치로 처리하고 아니면 컬럼명1의 내용으로 한다</br>
> 그 내용을 다시 컬럼명1에 저장한다
...
```
>조건이 여러개 일 땐, 아닐 거짓일 때 입력값에 조건을 추가하면 된다.</br>
>괄호를 잘 닫는 것 잊지 말자</br>
...
paste('안','녕','?')
```
>paste0는 ,를 다 붙여서 출력</br>
>paste는 ,를 다 띄여서 출력

# 6.arrange(정렬)
* 기본으로 오름차순 정렬
* %>% desc 를 사용하면 내림차순 정렬 사용할 수 있음

```R
df %>%
  arrange(나이, 수입 %>% desc)
```
> 나이로 오름차순 정렬, 수입으로 내림차순 정렬
